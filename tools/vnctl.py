...
def brief(root: Path, config: dict[str, Any]) -> int:
    ...
    print("Художественная вычитка: Russian-only schema 1; schema 2-4 — только retired history")
...
STYLE_EVENTS = {
    ...
    "style_retirement_annotated", "run_completed", "route_audited",
    "build_readback",
}
...
def style_run_complete(root: Path, config: dict[str, Any], route: str) -> bool:
    ...
    if not run or run.get("retired") or not run.get("audit"):
        return False
...
def work_queue(root: Path, config: dict[str, Any]) -> int:
    ...
    active_style_routes = {
        ...
        if run.get("route") and not run.get("retired") and not run.get("audit")
    }
    done = 0
    total_segments = 0
    ...
    for route, scenes in sorted(route_scenes(root, config).items()):
        ...
        run = current_style_run(root, config, route)
        if run and run.get("retired"):
            style_items.append(f"{route} ({len(rows)}, начать заново)")
        elif run and not run.get("audit"):
...
def style_start(root: Path, config: dict[str, Any], route: str) -> int:
    ...
    if current and not current.get("retired") and not current.get("audit"):
...
def style_status(root: Path, config: dict[str, Any]) -> int:
    ...
    for run in runs.values():
        audit = "retired" if run.get("retired") else (
            "audited" if run.get("audit") else "pending audit")
...
def validate_style_ledger(root: Path, config: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    for index, event in enumerate(events, start=1):
        ...
        if kind not in STYLE_EVENTS:
            ...
            continue
        if kind in {
                "style_keep_reverted", "style_siblings_reconciled",
                "style_run_retired", "style_retirement_annotated", "run_completed",
        } and schema_version == 1:
            errors.append(
                f"style ledger line {index}: migration event requires historical schema")
        if kind == "run_started":
            run_id = str(event.get("run_id", ""))
            if run_id in runs:
            ...
            runs[run_id] = {
                ...
                "applied_ids": {}, "accepted_keeps": {}, "keep_reverted": {},
            ...
            if kind == "window_applied":
                ...
                change_ids = {
                    str(change.get("id", "")) for change in event.get("changes", []) or []
                }
                if (not reverts or len(revert_ids) != len(set(revert_ids))
                        or any(not str(item.get("reason", "")).strip() for item in reverts)
                        or not set(revert_ids) <= run["accepted_keeps"].get(window_id, set())
                        or set(revert_ids) & previous or change_ids != set(revert_ids)):
                    errors.append(
                ...
                if (run["retired"] or not decisions
                    ...
                    errors.append(f"style ledger line {index}: invalid style retirement")
                reverted_ids = {
                    str(item.get("id", "")) for item in decisions
    ...
    for run_id, run in runs.items():
        if run["schema_version"] != 1 and not run["retired"]:
            errors.append(
                f"style run {run_id}: non-schema-1 run must be retired historical state")
...
REVIEW_EVENTS = {
    ...
    "review_invalidated", "review_finalization_blocked", "review_run_superseded",
}
...
def validate_review_ledger(root: Path, config: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    for index, event in enumerate(load_review_events(root, config), start=1):
        ...
        if kind not in REVIEW_EVENTS:
            ...
            continue
        migration_only = {"review_issue_reverted", "review_run_superseded"}
        if schema_version == 2 and kind not in migration_only:
            errors.append(
                f"review ledger line {index}: schema 2 is reserved for terminal migration")
        if kind in migration_only and schema_version != 2:
            errors.append(
                f"review ledger line {index}: migration event requires schema 2")
