...
def test_retired_historical_run_allows_new_schema1_style_run(tmp_path):
    root = Path(__file__).parents[2]
    vnctl = load_module(root / "tools/vnctl.py", "vnctl_retired_successor_test")
    config = make_project(tmp_path)
    assert vnctl.style_start(tmp_path, config, "BLK0002") == 0
    run_id = "STYLE-BLK0002-01"
    route_rows = vnctl.style_route_rows(tmp_path, config, "BLK0002")
    route_hash = vnctl.style_text_hash(route_rows)
    vnctl.append_style_event(tmp_path, config, {
        "schema_version": 4,
        "event": "style_run_retired",
        "run_id": run_id,
        "route": "BLK0002",
        "mode": "selective_exact_rollback",
        "actor": "vn-orchestrator",
        "reason": "Restore the reviewed baseline before a new Russian-only pass.",
        "request_sha256": "sha256:request",
        "input_route_sha256": route_hash,
        ...
        "changes": [],
        "result_route_sha256": route_hash,
    })

    assert vnctl.validate_style_ledger(tmp_path, config) == ([], [])
    assert vnctl.style_start(tmp_path, config, "BLK0002") == 0
    successor = vnctl.current_style_run(tmp_path, config, "BLK0002")
    assert successor["run_id"] == "STYLE-BLK0002-02"
    assert successor["schema_version"] == 1


def test_active_non_schema1_style_run_is_rejected(tmp_path):
    root = Path(__file__).parents[2]
    vnctl = load_module(root / "tools/vnctl.py", "vnctl_active_tep_rejection_test")
    config = make_project(tmp_path)
    assert vnctl.style_start(tmp_path, config, "BLK0002") == 0
    events = vnctl.load_style_events(tmp_path, config)
    events[-1]["schema_version"] = 4
    write_jsonl(tmp_path / "translation/style-ledger.jsonl", events)

    errors, warnings = vnctl.validate_style_ledger(tmp_path, config)
    assert warnings == []
    assert any("non-schema-1 run must be retired" in error for error in errors)
